...
```
Como nos pasaba en el clustering no jerárquico, también nos dice que el número óptimo es 1 cluster

```{r}
 nb2 <-  NbClust(sampled_data, distance = "euclidean", min.nc = 2, max.nc = 10, method = "ward.D2")
```

Según esto, nuestro número ideal de clusters es 2, como podemos ver en las gráficas. 
...
```{r}
set.seed(100)
fviz_cluster(list(data=datos_escalados,cluster=sub_grp))
```
