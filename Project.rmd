...
Las conclusiones que podemos sacar del EDA podemos decir que de una manera u otra pueden ser algunas más lógicas que ellas pero nos debemos cerciorar de que nuestras hipótesis y pensamientos pueden ser respaldados por datos.

Primeramente el cerciorarse de que no hay valores faltantes en nuestros datos ya que el hecho de que falten datos podría afectar a nuestro modelo para entender los datos y sacar conclusiones de ellos.
Al observar las distribuciones de nuestro dataset, hemos visto que hay variables continuas que no nos dan ninguna información añadida como active_member que al ser una variable binomial su distribución es bastante fácil de observar, por ello hemos usado esa funcion en ver cómo balance, credit_score y estimated_salary, como dijimos antes, balance sigue una distribución normal fuera de los ceros que tiene el dataset, si eliminamos los ceros vemos una variable normal de la misma manera credit_score.

Podemos observar que la tasa de personas que se mantienen en el banco es mayor que la de la gente que se va, varios gráficos más abajo vemos las relaciones de cada variable con el abandono del banco y podemos ver como lo más relacionado con el abandono es la edad, respecto a eso poco se puede hacer porque vemos que tenemos clientes que llevan mas de años en el banco por lo que vemos una gran fidelización de nuestra clientela pero siempre mejorable, pero el segundo dato con mayor relación es balance, la variable balance muestra el dinero restante de cada cliente en el banco y vemos que esa variable si que tiene una mayor relación con el abandono
...
La relación del dinero con el banco es muy interesante, no solo el dinero que tienen en el banco si no el salario que estimamos que tiene cada cliente con su permanencia con nosotros, podemos ver que realmente el salario estimado no tiene relacion alguna ni con el abandono ni con la permanencia de nuestro cliente, mientras que el balance y el dinero que tengan en el banco sí que lo es, ya que a mayor balance, podemos observar una mayor tendencia a abandonar el banco.

Respecto a la edad y el tiempo que llevan en el banco vemos que influye, a mayor edad la tasa de abandono es mayor, es algo que la empresa debería mejorar y obtener mejores resultados de estas variables.
