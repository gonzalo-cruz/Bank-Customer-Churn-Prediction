...
Con esta técnica, en nuestros datos, parece ser que la mejor elección es k igual a 2.
...
Un valor positivo significa que laobservación está bien agrupada, cuanto más se acerque el coeficiente a 1, mejor agrupada está la observación. En cambio un valor negativo significa que la observcaión está mal agrupada. Si una observación es igual a 0 significa que está entre dos conglomerados.
...
```
En este método hace lo siguiente: se aplica el algoritmo de clustering a los datos con diferentes valores de k. Se generan conjuntos de datos de referencia aleatorios. Se calcula el estadístico de Gap para cada valor de k. Se selecciona el número óptimo de clusters(el valor de k que maximiza el estadístico se considera el número óptimo de clusters).
...
Parece que el número óptimo de clusters es k = 6.

```{r}

k6 = kmeans(datos_escalados, centers = 6, nstart = 25)
