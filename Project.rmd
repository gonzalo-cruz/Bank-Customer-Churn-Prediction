...
validate <- Data[indices.val,]test <- Data[indices.test,]
...
```{r}
table(train$balance) #igual deberíamos plantearnos binarizarla -> ganamos robustez, perdemos info
```
Tenemos muchos valores en 0 de balance
Cambiar histograma a bar
```{r}
train_long <- train %>%
