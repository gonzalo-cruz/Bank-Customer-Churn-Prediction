...
##CODIGO JORGE, ESTROPEA EL K-NN

Estudiamos la influencia de active_member en churn
```{r}
tabla1 <- xtabs(~active_member+churn, data = train_data)
tabla1
```
...
```{r}
train_data$factor.churn <- factor(train_data$churn)
logit1 <- glm(factor.churn ~ active_member, data = train_data, family = "binomial")
summary(logit1)
```
...
bank_data$country <- as.factor(bank_data$country)
bank_data$credit_card <- as.factor(bank_data$credit_card)
bank_data$active_member <- as.factor(bank_data$active_member)
...
factor.churn <- factor(bank_data$churn)
...
```{r}
modelo2 <- glm(factor.churn ~ age + country + gender + balance + products_number + active_member, data = train_data, family = "binomial")
summary(modelo2)
```
...
train_data$churn <- ifelse(train_data$churn %in% c("yes", "1"), "yes", "no")
...
train_data_bal <- datos_balanceados[trainIndex, ]
test_data_bal <- datos_balanceados[-trainIndex, ]

modelo_log <- glm(factor.churn ~ credit_score + gender + age + tenure + balance +
                    products_number + credit_card + active_member + estimated_salary +
                    country,
                  data = train_data_bal, family = "binomial")

predicciones_prob <- predict(modelo_log, test_data_bal, type = "response")
predicciones <- ifelse(predicciones_prob > 0.5, "1", "0")

predicciones <- factor(predicciones, levels = c("0", "1"))
reales <- test_data_bal$factor.churn
...
```{r}
library(pROC)
roc_score=roc(test_data_bal$churn, predicciones_prob,plot=TRUE,print.auc=TRUE)
```
