...
```{r}
# Seleccionar solo variables numéricas para este ejemplo (excluyendo customer_id)
variables_numericas <- c("credit_score", "age", "tenure", "balance", "products_number", "estimated_salary")

X <- train_data[, variables_numericas]
y <- factor(train_data$churn, levels = c("X0", "X1"), labels = c("no", "yes"))
...
Dividimos los datos en grupos
```{r}
# Datos para churn = 0 (no abandono)
X0 <- X[y == "no", ]
# Datos para churn = 1 (abandono)
X1 <- X[y == "yes", ]
...
```{r}
# Matriz de covarianza para churn = 0
S0 <- cov(X0)
# Matriz de covarianza para churn = 1
S1 <- cov(X1)

# Matriz de covarianza agrupada 
S_agrupada <- ((n0 - 1) * S0 + (n1 - 1) * S1) / (n_total - 2)

# Verificación y regularización si es necesario
if (det(S_agrupada) == 0 || any(is.na(S_agrupada))) {
  S_agrupada <- S_agrupada + diag(1e-6, ncol(S_agrupada))
}
```

Calculamos los coeficientes discriminantes
```{r}
# Invertir la matriz de covarianza agrupada
S_agrupada_inv <- solve(S_agrupada)

# Coeficientes del discriminante lineal
coeficientes <- S_agrupada_inv %*% (mu1 - mu0)
names(coeficientes) <- variables_numericas

# Término constante
constante <- -0.5 * t(mu1 - mu0) %*% S_agrupada_inv %*% (mu1 + mu0) + log(n1/n0)
```

Función discriminante
```{r}
# Función para calcular los scores discriminantes
calcular_score <- function(x) {
  t(coeficientes) %*% x + constante
}

# Aplicar a todos los datos
scores <- apply(X, 1, calcular_score)

summary(scores)
sum(is.na(scores))
```

Clasificamos las observaciones basadas en los scores.
```{r}
# Umbral de decisión (usando probabilidades previas)
umbral <- 0.5
predicciones <- ifelse(scores > 0, "yes", "no")

# Matriz de confusión
matriz_confusion <- table(Predicho = predicciones, Real = y)
print(matriz_confusion)

# Precisión
precision <- sum(diag(matriz_confusion)) / n_total
print(paste("Precisión:", precision))
```

087 clientes que no hicieron churn fueron correctamente clasificados como "no": Verdaderos negativos.

135 clientes que sí hicieron churn fueron correctamente clasificados como "yes": Verdaderos positivos.

294 clientes fueron predichos como "sí" pero en realidad no abandonaron: Falsos positivos.

1484 clientes que sí abandonaron fueron predichos como "no": Falsos negativos.

- El modelo acierta en un 77.8% de los casos. El modelo acierta bastante bien en detectar los clientes que no van a abandonar el banco, sin embargo, falla bastante en detectar los que si abandonan, por lo que tenemos un desbalanceo de clases.


Para visualizar, hemos reducido a 2 dimensiones usando los dos primeros coeficientes.
```{r}
# Proyección en el espacio discriminante
proyeccion <- as.matrix(X) %*% coeficientes
...
# Graficar
library(ggplot2)
ggplot(df_plot, aes(x = LD1, fill = Churn)) +
  geom_histogram(alpha = 0.5, position = "identity", bins = 30) +
  labs(title = "Distribución de los scores discriminantes",
       x = "Score discriminante (LD1)", y = "Frecuencia") +
  theme_minimal()
```

- El eje X muestra los valores del score discriminante, es decir, el resultado de aplicar la función lineal construida a cada observación
- El eje y la frecuencia de clientes con ese score.

- Los clientes que no abandonaron están más concentrados en valores bajos del score, los que si abandonaron tienden a tener valores más altos, aunque se solapan bastante. Esto significa que el LDA no logra separar perfectamente a los clientes que se quedan de los que se van.
- Este solapamiento explica por qué la precisión del modelo no es muy alta.

Calculamos los coeficientes estandarizados para comparar la importancia de las variables.
```{r}
# Coeficientes estandarizados
desviaciones <- apply(X, 2, sd)
coeficientes <- as.numeric(coeficientes)
names(coeficientes) <- variables_numericas

coeficientes_estandarizados <- coeficientes * desviaciones


# Ordenar por importancia
importancia <- data.frame(
  Variable = names(coeficientes_estandarizados),
  Coeficiente = as.numeric(coeficientes_estandarizados),
  Absoluto = abs(as.numeric(coeficientes_estandarizados))
)

importancia <- importancia[order(-importancia$Absoluto), ]
print(importancia)
```

Age: 0.76: Es la variable más importante. A mayor edad, mayor probabilidad de churn.

Balance: 0.29: Segunda en importancia. Clientes con más saldo tienen mayor probabilidad de churn.

Credit_score: -0.08: Tiene un efecto leve, y negativo: a mayor puntuación crediticia, menor probabilidad de churn.

Estimated_salary: 0.04: Efecto pequeño: los salarios más altos se asocian ligeramente con más churn.

Products_number: -0.02: Casi irrelevante. Tener más productos se asocia muy débilmente con menos churn.

Tenure: 0.005: No aporta prácticamente nada.
