...
bagging_model <- randomForest(
  Species ~ ., 
  data = train,
  mtry = ncol(train) - 1,  # todas menos la variable objetivo
  importance = TRUE
)
...
conf_matrix_bagging <- confusionMatrix(
  data = bagging_preds,
...
| **Kappa** | 0.9 | Excelente acuerdo entre predicción y realidad (más allá del azar). |
| **Setosa - Sensibilidad** | 1.000 | El modelo identifica correctamente todos los casos de setosa. |
| **Setosa - Especificidad** | 1.000 | No hay falsos positivos al predecir setosa. |
| **Versicolor - Sensibilidad** | 0.9333 | La mayoría de versicolor se identifican correctamente. |
| **Versicolor - Especificidad** | 0.9333 | Muy pocos falsos positivos para versicolor. |
| **Virginica - Sensibilidad** | 0.8667 | Buen desempeño, aunque hay algunas confusiones. |
| **Virginica - Especificidad** | 0.9667 | Casi sin falsos positivos al predecir virginica. |
| **Variable más importante (Accuracy)** | Petal.Width | Es la que más influye en la precisión del modelo. |
| **Variable más importante (Gini)** | Petal.Width | Es la que más reduce la impureza en los árboles. |
| **Variables con menor importancia** | Sepal.Width, Sepal.Length | Aportan poco al modelo, se podrían eliminar sin gran pérdida. |
| **Clase más fácilmente identificable** | Setosa | Totalmente separable por el modelo. |
| **Mayor confusión entre clases** | Versicolor y Virginica | Son las más parecidas, el modelo tiende a confundirlas. |
| **Conclusión general** | Modelo preciso, balanceado y estable | Excelente rendimiento general, buena interpretación y separación de clases. |
